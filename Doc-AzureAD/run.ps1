...
$HuduBaseDomain = "https://e3itservices.huducloud.com/"
...
$upn = 'M365hudu@RCComputersLLC.onmicrosoft.com'
...
AzureADPreview\Connect-AzureAD -AadAccessToken $aadGraphToken.AccessToken -AccountId $UPN -MsAccessToken $graphToken.AccessToken -TenantId $tenantID | Out-Null
...
foreach ($Customer in $Customers) {
	...
	if (-Not ($customerExclude -contains $customer.DisplayName)){
		...
		if ($($hududomain.id.count) -gt 0) {
			...
			AzureADPreview\Connect-AzureAD -AadAccessToken $CustAadGraphToken.AccessToken -AccountId $upn -MsAccessToken $CustGraphToken.AccessToken -TenantId $customer.CustomerContextId | out-null
...
$authorizationHeader = @{
    Authorization = $token.CreateAuthorizationHeader()
			...
			$requestURL = "https://graph.microsoft.com/beta/deviceAppManagement/mobileAppConfigurations"
			$Applications = Invoke-RestMethod -Uri $requestURL -Headers $authorizationHeader
			$Applications
